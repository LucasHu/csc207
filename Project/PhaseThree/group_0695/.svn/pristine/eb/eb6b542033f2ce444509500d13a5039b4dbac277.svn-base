...
public class ClientInfoActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        clientsManager = (ClientManager) 
        		intent.getSerializableExtra(MainActivity.CLIENT_MANAGER_KEY);
        ...
        if(clientsManager.getClients().containsKey(currentUser)){
            ...
            TextView creditCardNumber = (TextView) 
            		findViewById(R.id.credit_card_number_field);
            creditCardNumber.setText("Credit card number: " + 
            currentClient.getCreditCardNumber());

            TextView expiracyDate = (TextView) 
            		findViewById(R.id.expiracy_date_field);
            expiracyDate.setText("Expiracy date: " + 
            		currentClient.getExpiryDate());
        }
        else {
