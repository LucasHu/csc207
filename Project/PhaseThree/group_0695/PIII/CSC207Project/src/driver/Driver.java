...
public class Driver {
	...
	public static void uploadClientInfo(String path) {
		...
		try {
			...
			tempAdmin.uploadClientFile(path);
	 ...
	 *            Destination,Price,NumSeats (the dates are in the format
	 *            YYYY-MM-DD HH:MM; the price has exactly two decimal places;
	 *            the number of seats is a non-negative integer)
	 */
	public static void uploadFlightInfo(String path) {

		try {

			tempAdmin.uploadFlightFile(path);
	...
	public static String getClient(String email) {
		...
		try {
			...
			allClients = tempAdmin.readClientFile();
	 ...
	 *         Destination,Price (the departure and arrival date and time are in
	 *         the format YYYY-MM-DD HH:MM; the price has exactly two decimal
	 *         places)
	 */
	public static String getFlights(String date, String origin,
		...
		try {
			...
			searchedFlights = tempAdmin.searchFlights(date, origin,
					destination);
	 ...
	 * possible itinerary (we judge that the stopover is too long). In addition,
	 * the itineraries returned do not contain cycles, i.e., they do not visit
	...
	public static String getItineraries(String date, String origin,
		...
		List<Itinerary> searchedItineraries = null;

		try {

			searchedItineraries = tempAdmin.searchedItinerariesList(date,
					origin, destination);
	...
	public static String getItinerariesSortedByCost(String date, String origin,
		...
		List<Itinerary> sortedSearchedItinerariesByCost = null;

		try {

			sortedSearchedItinerariesByCost = tempAdmin
	...
	public static String getItinerariesSortedByTime(String date, String origin,
		...
		List<Itinerary> sortedSearchedItinerariesByTime = null;

		try {
			sortedSearchedItinerariesByTime = tempAdmin
